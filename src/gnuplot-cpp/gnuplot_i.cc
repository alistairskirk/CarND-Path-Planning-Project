...
#include <iostream> // for debug Mauro
...
//------------------------------------------------------------------------------
//
// Plots a 2d graph from a list of doubles (x y) and labels saved in a file
//
Gnuplot& Gnuplot::plotfile_xy_labels(const std::string &filename,
	const unsigned int column_x,
	const unsigned int column_y,
	const unsigned int column_l,
	const std::string &title)
{
	//
	// check if file exists
	...
	cmdstr << "\"" << filename << "\" using " << column_x << ":" << column_y <<":" <<column_l <<" with labels";
...
std::string Gnuplot::create_tmpfile(std::ofstream &tmp)
{
    ...
    //
    //std::cout << "Save file name: " << name << std::endl;
    tmpfile_list.push_back(name);
    //std::cout << "Size is: "<< tmpfile_list.size()<< std::endl;
    Gnuplot::tmpfile_num++;
...
void Gnuplot::remove_tmpfiles(){
    //std::cout << "remove temp files" << tmpfile_num << " "<< tmpfile_list.size()<<std::endl;
    if ((tmpfile_list).size() > 0)
    {
       //std::cout << "remove temp files in the if" << std::endl;
