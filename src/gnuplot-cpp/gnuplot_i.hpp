...
class Gnuplot
{
    ...
    Gnuplot& plotfile_xy(const std::string &filename,
                         ...
                         const std::string &title = "");
	/// plot x,y pairs with label: x y label
	///   from file
	Gnuplot& plotfile_xy_labels(const std::string &filename,
		const unsigned int column_x = 1,
		const unsigned int column_y = 2,
		const unsigned int column_l = 3,
		const std::string &title = "");
	...
	Gnuplot& plot_xy_labels(const X& x, const Y& y, const L& label, const std::string &title = "");
...
//------------------------------------------------------------------------------
//
/// Plots a 2d graph from a list of doubles: x y, and a list of string labels for each coord
//
template<typename X, typename Y, typename L>
Gnuplot& Gnuplot::plot_xy_labels(const X& x, const Y& y, const L& label, const std::string &title)
{
	...
	plotfile_xy_labels(name, 1, 2, 3, title);
